...
namespace DeliveryGamemode
{
	...
	public class Hatchback : CarEntity
	{
		...
		public override void Spawn()
		{
			...
			passenger_st.usable = delegate ( Entity ent ) { return this.IsUsableAsPassenger( ent ); };
			passenger_st.use = delegate ( Entity ent ) { return this.SitAsPassenger( ent ); };
			{
				Vector3 size = Vector3.One * 30;
				Vector3 pos = Vector3.Up * 15;
				passenger_st.SetupPhysicsFromOBB( PhysicsMotionType.Keyframed, -(size / 2) + pos, size / 2 + pos );
				passenger_st.SetInteractsExclude( CollisionLayer.PhysicsProp );
			}
		...
		public override bool SitAsPassenger( Entity user )
		{
			if ( user is DeliveryPlayer player && player.Vehicle == null && timeSincePassengerLeft > 1.0f )
			{
				...
				player.VehicleCamera = new CarCamera();
				player.Parent = this;
				player.LocalPosition = Vector3.Up * 16 + Vector3.Forward * 4 + Vector3.Right * 15;
				player.LocalRotation = Rotation.From( -12, 0, 0 );
				player.LocalScale = 1;
				player.PhysicsBody.Enabled = false;
